...
function getImages() {
  $
          .ajax({
            type: 'get',
            url: "/" + projectName + "/image/getImages",
            data: {},
            async: false,
            success: function(msg) {
              ...
              if (obj.result == '1') {
                ...
                for ( var val in obj) {
                  ...
                  var path = '../../../../' + projectName + obj[val].directory
                          + '/' + obj[val].imagename;
                  var dir = obj[val].directory + '/' + obj[val].imagename;
                  /*
                   * var path = projectName + obj[val].directory + '/' +
                   * obj[val].imagename;
                   */
                  var title = obj[val].title == undefined ? "点击添加标题"
                          : obj[val].title;
                  var content = obj[val].describecontent == undefined
                          ? "点击添加描述" : obj[val].describecontent;
                  ...
                  // '<input type="hidden" class="pictureid"
                  // value="'+pictureid+'"/>'+
                  // '</figcaption>'+
                  // '</figure>';
                  var html = '<div class="item col-md-2 col-sm-6 col-xs-12">'
                          + '<img alt="" src="'
                          + path
                          + '" alt="NAME" class="img-responsive img-rounded carousel-inner">'
                          + '<a href='
                          + path
                          ...
                          + '</div>'
                          + '</a>'
                          + '<input type="hidden" class="pictureid" value="'
                          + pictureid
                          + '"/>'
                          + '</div>'
                          + '<div class="item col-md-2 col-sm-6 col-xs-12">'
                          + '<button type="button" class="btn btn-danger btn-sm del-btn">删除</button>'
                          + '<button type="button" class="btn btn-danger btn-sm del-btn" id="'
                          + dir
                          + '"  onclick="download(this)">下载</button>'
                          + '<div>'
...
var editPicture = function() {
  $('.figurebase').click(
          function() {
            ...
            var title = obj.children('.title').text() == "点击添加标题" ? "" : obj
                    .children('.title').text();
            var content = obj.children('.content').text() == "点击添加描述" ? ""
                    : obj.children('.content').text();
...
function download(obj) {
  ...
  $.ajax({
    type: "get",
    url: "/" + projectName + "/download/download",
    data: {
      "path": path
    },
    success: function(msg) {
